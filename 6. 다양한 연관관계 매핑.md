...
- 일대일 관계이므로 객체 매핑에 `@OneToOne`을 사용했고 데이터베이스는 `LOCKER_ID` 외래 키에 유니크 제약 조건(UNI)을 추가했다.
...
- 반대 매핑인 사물함의 `Locker.member`는 `mappedBy`를 선언해서 연관관계의 주인이 아니라고 설정했다.

> 프록시를 사용할 때 외래 키를 직접 관리하지 않는 일대일 관계는 지연 로딩으로 설정해도 즉시 로딩된다. 이것은 프록시의 한계 때문에 발생하는 문제인데 프록시 대신에 bytecode instrumentation을 사용하면 해결할 수 있다.
